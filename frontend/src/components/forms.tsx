...
// Also disables hidden fields so they don't trigger validation errors.
    ...
    useEffect(() => {
        ...
        if(b) {
            inputs.forEach(inp => {
                ...
                if (closestShow === el || (closestShow as HTMLElement)?.dataset.show !== '0') {
                    input.disabled = false;
                }
            });
        } else {
            inputs.forEach(inp => (inp as HTMLInputElement).disabled = true);
        }
        prev.current = b;
    }, [ref, b]);
    return <div data-ind={indent} ref={ref} data-show={b ? "1" : "0"}><div>
        { children }
    </div></div>;
}

// A semi-controlled form wrapper component. The field values are passed in as
// the "initial" prop, which should only be changed when the form is first
// mounted, or the underlying data changes (eg after form submit).
//
...
export function ipField(label: string, name: string, placeholder?: string) {
    return <div class="form-row">
        <label>{ label }</label>
        <input type="text" name={ name } placeholder={ placeholder || undefined } pattern="|((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)" title="IPv4 address or blank" />
    </div>;
}
