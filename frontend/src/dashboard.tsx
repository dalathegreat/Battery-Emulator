...
import { SocMeter } from './components/socmeter';
...
const CONTACTOR_STATE_STATUS = ["e", "d", "d", "g", "g", "g", "e"]; // error, danger, good
  ...
  return <div>
    ...
    {status.contactor &&
      <div class="panel" data-status={CONTACTOR_STATE_STATUS[status.contactor.state]}>
        <div class={"badge " + CONTACTOR_STATE_STATUS[status.contactor.state]} style="float: right">{CONTACTOR_STATE_NAME[status.contactor.state]}</div>
      ...
      <div class="panel" key={idx} data-status={battery.status?.toLowerCase()}>
        ...
        <div class="stats">
          <div class="stat-circle">
            ...
            {battery.i > 0.05 ?
              <span class="badge c s">CHARGING</span>
              : battery.i < -0.05 ?
                <span class="badge d s">DISCHARGING</span>
                :
                <span class="badge i">IDLE</span>
            }
          ...
          {(battery.temp_min >= 10 || battery.temp_max >= 20) &&
            ...
            <div style="background: #1a1a1a; border-radius: 6px; padding: 0.4rem; font-size: 10px; line-height: 1.2; letter-spacing: 1px">
              SOC SCALING: 20/90%<br />
              CELLS: {battery.cell_mv_min}-{battery.cell_mv_max} mV ({battery.cell_mv_max - battery.cell_mv_min}mV)<br />
            </div>
