 ...
 * @api private
 */
function pathtoRegexp(path: string | RegExp | Array<any>, keys: Array<any>, options: OptionsType): RegExp {
	options = options || {};
	keys = keys || [];
...
export function EvaluateParams(path: string, c2regexp: { regexp: RegExp; groups: string[] }) {
	...
	if (r != null) {
		...
		return result; // /:username/job/:jid ==> { username : "herberth", "jid" : 1234 }
	}
}

export function pathToRegExpAndGroups(path: string): { regexp: RegExp; groups: string[] } {
	try {
		let keys: any[] = [],
			finalKeys: string[] = [];
