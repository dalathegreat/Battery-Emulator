...
// Loads the `marked` library from a CDN and renders the release notes as HTML;
// falls back to plain pre-formatted text while (or if) the library is
// unavailable. A component (not a function calling hooks) so hook usage is
// stable regardless of how many releases are listed.
function Markdown({ text }: { text: string }) {
    ...
    useEffect(() => {
        ...
        import(/* @vite-ignore */ "https://cdn.jsdelivr.net/npm/marked/lib/marked.esm.js")
            .then(m => { if (!cancelled) setMarked(m); })
