...
String VolvoSpaHtmlRenderer::get_status_html() {
  String content;
  ...
  content += "<h4>Discharge power limit 1: " + String(datalayer_extended.VolvoPolestar.HvBattPwrLimDcha1) + " kW</h4>";
  content +=
  ...
  content +=
      ...
      " kW</h4>";
  content += "<h4>Charge power limit slow aging: " + String(datalayer_extended.VolvoPolestar.HvBattPwrLimChrgSlowAgi) +
             " kW</h4>";
...
String VolvoSpaHtmlRenderer::render_dtc_section(DATALAYER_BATTERY_DTC_TYPE& dtc) {
  String content;
  ...
  for (uint8_t i = 0; i < dtc.dtc_count; i++) {
    ...
    snprintf(matchKey, sizeof(matchKey), "%c%02lX%02lX", SYS[(code >> 22) & 0x03], (unsigned long)((code >> 16) & 0x3F),
             (unsigned long)((code >> 8) & 0xFF));
