...
String TeslaHtmlRenderer::get_status_html() {
  const DATALAYER_INFO_TESLA& tesla = *tesla_info;
    String content;
    ...
    float dcdcLvBusVolt = static_cast<float>(tesla.battery_dcdcLvBusVolt) * 0.01f;
    float dcdcHvBusVolt = static_cast<float>(tesla.battery_dcdcHvBusVolt) * 0.1f;
    ...
    float total_discharge = static_cast<float>(core_data->status.total_discharged_battery_Wh) * 0.001f;
    float total_charge = static_cast<float>(core_data->status.total_charged_battery_Wh) * 0.001f;
    float packMass = static_cast<float>(tesla.battery_packMass);
    float platformMaxBusVoltage = static_cast<float>(tesla.battery_platformMaxBusVoltage);
    ...
    content += "<h4>Battery Manufacture Date: " + String(tesla.battery_manufactureDate) + "</h4>";
    ...
    content +=
        "<h4>HVP Contactor State: " + String(contactorText[tesla.packContactorSetState]) + "</h4>";
    content +=
        "<h4>BMS Contactor State: " + String(BMS_contactorState[tesla.BMS_contactorState]) + "</h4>";
    content +=
        "<h4>Negative Contactor: " + String(contactorState[tesla.packContNegativeState]) + "</h4>";
    content +=
        "<h4>Positive Contactor: " + String(contactorState[tesla.packContPositiveState]) + "</h4>";
    ...
    content += "<h4>Contactors Request Status: " +
               String(HVP_contactor[tesla.battery_packCtrsRequestStatus]) + "</h4>";
    ...
    content += "<h4>BMS UI Charge Status: " + String(BMS_uiChargeStatus[tesla.BMS_uiChargeStatus]) +
               "</h4>";
    ...
    appendFault(content, "BMS PCS PWM Enabled", tesla.BMS_pcsPwmEnabled);
    ...
    content +=
        "<h4>Power Limit State: " + String(BMS_powerLimitState[tesla.BMS_powerLimitState]) + "</h4>";
    ...
    content +=
        "<h4>Precharge Status: " + String(PCS_dcdcStatus[tesla.PCS_dcdcPrechargeStatus]) + "</h4>";
    content += "<h4>12V Support Status: " + String(PCS_dcdcStatus[tesla.PCS_dcdc12VSupportStatus]) +
               "</h4>";
    content += "<h4>HV Bus Discharge Status: " +
               String(PCS_dcdcStatus[tesla.PCS_dcdcHvBusDischargeStatus]) + "</h4>";
    ...
    content +=
        "<h4>Precharge Rty Cnt: " + String(falseTrue[tesla.PCS_dcdcPrechargeRtyCnt]) + "</h4>";
    content +=
        "<h4>12V Support Rty Cnt: " + String(falseTrue[tesla.PCS_dcdc12VSupportRtyCnt]) + "</h4>";
    content +=
        "<h4>Discharge Rty Cnt: " + String(falseTrue[tesla.PCS_dcdcDischargeRtyCnt]) + "</h4>";
    ...
    content += "<h4>Precharge Restart Cnt: " + String(falseTrue[tesla.PCS_dcdcPrechargeRestartCnt]) +
               "</h4>";
    content += "<h4>Initial Precharge Substate: " +
               String(PCS_dcdcSubState[tesla.PCS_dcdcInitialPrechargeSubState]) + "</h4>";
    //0x3C4 PCS_info
    ...
    appendFault(content, "HVP_gpioPassivePyroDepl", tesla.HVP_gpioPassivePyroDepl);
    appendFault(content, "HVP_gpioPyroIsoEn", tesla.HVP_gpioPyroIsoEn);
    appendFault(content, "HVP_gpioCpFaultIn", tesla.HVP_gpioCpFaultIn);
    appendFault(content, "HVP_gpioPackContPowerEn", tesla.HVP_gpioPackContPowerEn);
    appendFault(content, "HVP_gpioHvCablesOk", tesla.HVP_gpioHvCablesOk);
    appendFault(content, "HVP_gpioHvpSelfEnable", tesla.HVP_gpioHvpSelfEnable);
    appendFault(content, "HVP_gpioLed", tesla.HVP_gpioLed);
    appendFault(content, "HVP_gpioCrashSignal", tesla.HVP_gpioCrashSignal);
    appendFault(content, "HVP_gpioShuntDataReady", tesla.HVP_gpioShuntDataReady);
    appendFault(content, "HVP_gpioFcContPosAux", tesla.HVP_gpioFcContPosAux);
    appendFault(content, "HVP_gpioFcContNegAux", tesla.HVP_gpioFcContNegAux);
    appendFault(content, "HVP_gpioBmsEout", tesla.HVP_gpioBmsEout);
    appendFault(content, "HVP_gpioCpFaultOut", tesla.HVP_gpioCpFaultOut);
    appendFault(content, "HVP_gpioPyroPor", tesla.HVP_gpioPyroPor);
    appendFault(content, "HVP_gpioShuntEn", tesla.HVP_gpioShuntEn);
    appendFault(content, "HVP_gpioHvpVerEn", tesla.HVP_gpioHvpVerEn);
    appendFault(content, "HVP_gpioFcContFlywheelEnable", tesla.HVP_gpioFcContFlywheelEnable);
    appendFault(content, "HVP_gpioCpLatchEnable", tesla.HVP_gpioCpLatchEnable);
    appendFault(content, "HVP_gpioFcContPowerEnable", tesla.HVP_gpioFcContPowerEnable);
    appendFault(content, "HVP_gpioHvilEnable", tesla.HVP_gpioHvilEnable);
    ...
    // web_data/dtc/README.md; the JSON "code" field must match "base + index" below.
    {
      ...
      const AlertGroup groups[] = {
          {"BMS 0x320", 100, tesla.BMS_alertMatrixActive, 100},
          {"PCS 0x3A4", 200, tesla.PCS_alertMatrixActive, 94},
          {"CP 0x31E", 300, tesla.CP_alertMatrixActive, 96},
      };
