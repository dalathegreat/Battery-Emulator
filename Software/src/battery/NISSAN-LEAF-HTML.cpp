...
String NissanLeafHtmlRenderer::get_status_html() {
  String content;
  if (!nissan_dl) {
    return content;
  }
  ...
  content +=
      "<h4>SolvedChallenge: " + String(nissan_dl->SolvedChallengeMSB) + String(nissan_dl->SolvedChallengeLSB) + "</h4>";
...
String NissanLeafHtmlRenderer::render_dtc_section(DATALAYER_BATTERY_DTC_TYPE& dtc) {
  String content;
