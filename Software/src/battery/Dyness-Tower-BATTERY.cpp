#include "DYNESS-TOWER-BATTERY.h"
#include "../battery/BATTERIES.h"
#include "../communication/can/comm_can.h"
#include "../datalayer/datalayer.h"
#include "../devboard/utils/events.h"

void DynessBattery::update_values() {
  ...
  // Use DYNESS protocol min/max for alarms
...
void DynessBattery::handle_incoming_can_frame(CAN_frame rx_frame) {
  ...
  switch (rx_frame.ID) {
    ...
    case 0x7311:
      ...
      //not sure if this is correct (copy of Pylon)
...
void DynessBattery::transmit_can(unsigned long currentMillis) {
  // Send 1s CAN Message
  if (currentMillis - previousMillis1000 >= INTERVAL_1_S) {
    previousMillis1000 = currentMillis;

    transmit_can_frame(&DYNESS_3010);  // Heartbeat
    transmit_can_frame(&DYNESS_4200);  // Ensemble OR System equipment info, depends on frame0
    transmit_can_frame(&DYNESS_8200);  // Control device quit sleep status
    transmit_can_frame(&DYNESS_8210);  // Charge command

    if (ensemble_info_ack) {
      DYNESS_4200.data.u8[0] = 0x00;  //Request system equipment info
    }
  }
...
void DynessBattery::setup(void) {  // Performs one time setup at startup
  strncpy(datalayer.system.info.battery_protocol, "Dyness Tower battery", 63);
