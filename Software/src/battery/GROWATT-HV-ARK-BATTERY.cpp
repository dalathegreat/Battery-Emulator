...
void GrowattHvArkBattery::transmit_can(unsigned long currentMillis) {
  ...
  // --- 0x3020 Control ---
  // When a contactor_closing_allowed flag is provided (secondary/tertiary batteries), respect it.
  const bool allow_powerflow = (contactor_closing_allowed == nullptr) ? true : *contactor_closing_allowed;
  PCS_3020.data.u8[0] = allow_powerflow ? 0xAA : 0x00;  // Charging command
  PCS_3020.data.u8[1] = allow_powerflow ? 0xAA : 0x00;  // Discharging command
