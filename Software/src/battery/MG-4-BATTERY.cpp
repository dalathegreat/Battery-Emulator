  ...
  switch (rx_frame.ID) {
    case 0x12C:
      ...
      datalayer.battery.status.real_soc =
          estimate_soc_from_cell((datalayer.battery.status.voltage_dV * 100) / datalayer.battery.info.number_of_cells);

      datalayer.battery.status.max_charge_power_W = 5000;
      if (datalayer.battery.status.real_soc > 9000) {
        datalayer.battery.status.max_charge_power_W *= (1.0f - ((datalayer.battery.status.real_soc - 9000) / 1000.0f));
      }
      datalayer.battery.status.max_discharge_power_W = 5000;
      if (datalayer.battery.status.real_soc < 1000) {
        datalayer.battery.status.max_discharge_power_W *= (datalayer.battery.status.real_soc / 1000.0f);
      }
