  ...
  switch (rx_frame.ID) {
    ...
    case 0x7BB: {
      ...
      if (frame_type == 0x00 && !isotp_busy) {
        ...
        if (sf_len < 2 || sf_len > 7 || sf_len > dlc - 1) break;
        // Positive response: data[1] = 0x61 or 0x62, data[2] = PID
        if ((d[1] != 0x61 && d[1] != 0x62) || sf_len < 2) break;
        ...
        if (total_len > sizeof(isotp_buf) || total_len < 2) break;
        ...
        const uint16_t bytes_to_copy = (isotp_exp_len - isotp_rxlen < 7)
                                           ? (isotp_exp_len - isotp_rxlen)
                                           : 7;
...
void SmartEq453Battery::process_isotp_complete(uint8_t pid, const uint8_t* data, uint16_t len) {
  ...
  switch (pid) {
    case 0x07: decode_pid07(data, len); last_pid07_ms = millis(); break;
    case 0x01: decode_pid01(data, len); break;
    case 0x08: decode_pid08(data, len); break;
    case 0x04: decode_pid04(data, len); break;
    case 0x29: decode_pid29(data, len); break;
    case 0x41: decode_pid41_42(data, len, 0); break;
    case 0x42: decode_pid41_42(data, len, 1); break;
    case 0x10: decode_pid10_11(data, len, 0); break;
    case 0x11: decode_pid10_11(data, len, 1); break;
    case 0x16: decode_pid16(data, len); break;
    case 0x25: decode_pid25(data, len); break;
    case 0x61: decode_pid61(data, len); break;
    case 0x0B: decode_pid0B(data, len); break;
    case 0x02: decode_pid02(data, len); break;
    case 0x80: decode_pid80(data, len); break;
    case 0xEF: decode_pidEF(data, len); break;
    case 0xA0: decode_pidA0(data, len); break;
    case 0xF0 /* pseudo-PID for 0x22/0x0304 production date */: decode_pid_22_0304(data, len); break;
    default: break;
  }
}
...
void SmartEq453Battery::decode_bcast_0x0C6(const uint8_t* d, uint8_t dlc) {
  ...
  if (v_raw < 20000 || v_raw > 45000) return;
...
void SmartEq453Battery::check_contactor_counter_safety() {
  if (!contactor_counter_valid) return;
...
void SmartEq453Battery::update_values() {
  ...
  if (uds_voltage_fresh) {
    ...
    datalayer_battery->status.voltage_dV =
        static_cast<uint16_t>(bcast_voltage_raw / 10);
  }

  // DC link voltage from broadcast 0x0C6 bytes 4-5 → extended datalayer for display
  if (bcast_voltage_valid) {
    datalayer_extended.smart453.bcast_dc_link_dV =
        static_cast<uint16_t>(bcast_dc_link_raw / 10);
  }
  ...
  datalayer_battery->status.current_dA =
      static_cast<int16_t>((static_cast<int32_t>(pack_current_raw) * 10) / 32);
  ...
  if (uds_soc_fresh) {
    ...
    if (soc_pptt > 10000) soc_pptt = 10000;
  ...
  if (charge_limit_raw > 0) {
    ...
    datalayer_battery->status.max_charge_current_dA =
        static_cast<uint16_t>(charge_limit_raw * 10 / 1024);
  }
  if (discharge_limit_raw < 0) {
    ...
    datalayer_battery->status.max_discharge_current_dA =
        static_cast<uint16_t>((-discharge_limit_raw) * 10 / 1024);
  }

  // ── Remaining capacity estimate
  datalayer_battery->status.remaining_capacity_Wh = static_cast<uint32_t>(
      (static_cast<double>(datalayer_battery->status.real_soc) / 10000.0) *
  ...
  const bool safety_ok =
      (hv_plug_interlock == 1) &&
      (service_disconnect == 1) &&
      (fusi_raw == 0) &&
      (safety_mode == 0) &&
      (relay_permit == 1) &&
      (contactor_counter_valid) &&
...
void SmartEq453Battery::update_state_machine(unsigned long now) {
  ...
  switch (state) {
    ...
    case Smart453State::BMS_RESPONDING:
      ...
      if      (vehicle_state_byte == 0xC3) state = Smart453State::STATE_C3;
      else if (vehicle_state_byte == 0xC4) state = Smart453State::STATE_C4;
      else if (vehicle_state_byte == 0xC5) state = Smart453State::STATE_C5;
      else if (vehicle_state_byte == 0xC6) state = Smart453State::STATE_C6;
      else if (vehicle_state_byte == 0xC7) state = Smart453State::STATE_C7_READY;
      break;

    case Smart453State::STATE_C3:
    case Smart453State::STATE_C4:
    case Smart453State::STATE_C5:
      if      (vehicle_state_byte == 0xC6) state = Smart453State::STATE_C6;
      else if (vehicle_state_byte == 0xC7) state = Smart453State::STATE_C7_READY;
      else if (vehicle_state_byte == 0xC0) state = Smart453State::BMS_RESPONDING;
      break;
...
bool SmartEq453Battery::poll_is_safe_to_send() const {
  ...
  return !isotp_busy && (state != Smart453State::FAULT) &&
         (state != Smart453State::LV_POWERED) &&
