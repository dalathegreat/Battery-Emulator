...
static bool alternate_100ms = true;
CAN_frame EGMP_F5 = {
    .FD = true,
    .ext_ID = false,
    .DLC = 32,
  ...
  if (startedUp) {
    ...
    */
    ...
    if (currentMillis - previousMillis200ms >= INTERVAL_200_MS) {
      ...
      transmit_can(&EGMP_2D5,
                   can_config.battery);  // Needed for contactor closing (Unsure if really 200ms, but works in log)
      transmit_can(&EGMP_27A,
                   can_config.battery);  // Needed for contactor closing (Unsure if really 200ms, but works in log)
      transmit_can(&EGMP_2EA,
                   can_config.battery);  // Needed for contactor closing (Unsure if really 200ms, but works in log)
      transmit_can(&EGMP_306,
                   can_config.battery);  // Needed for contactor closing (Unsure if really 200ms, but works in log)
      transmit_can(&EGMP_308,
                   can_config.battery);  // Needed for contactor closing (Unsure if really 200ms, but works in log)
      transmit_can(&EGMP_33A,
                   can_config.battery);  // Needed for contactor closing (Unsure if really 200ms, but works in log)
      transmit_can(&EGMP_350,
                   can_config.battery);  // Needed for contactor closing (Unsure if really 200ms, but works in log)
      transmit_can(&EGMP_2E5,
                   can_config.battery);  // Needed for contactor closing (Unsure if really 200ms, but works in log)
      transmit_can(&EGMP_255,
                   can_config.battery);  // Needed for contactor closing (Unsure if really 200ms, but works in log)
      transmit_can(&EGMP_3B5,
                   can_config.battery);  // Needed for contactor closing (Unsure if really 200ms, but works in log)
      transmit_can(&EGMP_2C0,
                   can_config.battery);  // Needed for contactor closing (Unsure if really 200ms, but works in log)
      /* COMMENTED OUT WHILE CONTACTOR CLOSING TESTING
