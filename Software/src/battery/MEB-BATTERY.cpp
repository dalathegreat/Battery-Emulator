...
void send_can_battery() {
  ...
  if (currentMillis - previousMillis100ms >= INTERVAL_100_MS) {
    ...
    if (datalayer.battery.status.bms_status != FAULT && /*first_can_msg > 0 && currentMillis > first_can_msg + 2000*/
      ...
      MEB_503.data.u8[3] = BMS_TARGET_AC_CHARGING;//BMS_TARGET_HV_ON;  //BMS_TARGET_AC_CHARGING;  //TODO, should we try AC_2 or DC charging?
      MEB_503.data.u8[5] = 0x82;              // Bordnetz Active
      MEB_503.data.u8[6] = 0xE0;              // Request emergency shutdown HV system == 0, false
    } else if (first_can_msg > 0 && currentMillis > first_can_msg + 2000 && BMS_mode != 0 && BMS_mode != 7 ) {  //FAULT STATE, open contactors
