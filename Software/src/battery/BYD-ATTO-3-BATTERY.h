...
class BydAttoBattery : public CanBattery {
 ...
 private:
  ...
  /* Native BMS termination (opt-in, see handle_charge_session). Runs a real AC charge session on the
  pack bus so the BMS ends the charge itself and recalibrates SOC to 100%, instead of BE stopping the
  charge at its own cell clamp. Needs the pack case isolated from earth: an insulation fault keeps the
  BMS out of the charge context. Everything here is inert while the feature is off. */
