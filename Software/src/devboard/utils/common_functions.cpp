...
const uint8_t crc8_table_SAE_J1850_ZER0[256] = {
    //0x1D Poly,initial value 0x3F,Final XOR value varies
...
const uint8_t crctable_crc8_2f[256] = {
    // CRC8 Autosar formula,0x2F Poly,initial value 0xFF,Final XOR value 0xFF
...
uint8_t Crc_CalculateCRC8H2F(const uint8_t* data, uint16_t length, uint8_t initial_value, uint8_t final_xor_value) {
  // AUTOSAR CRC8H2F: 0x2F poly, non-reflected input/output.
  // The AUTOSAR defaults are initial value 0xFF and final XOR value 0xFF
  uint8_t crc = initial_value;

  for (uint16_t i = 0; i < length; i++) {
    crc = crctable_crc8_2f[crc ^ data[i]];
  }
