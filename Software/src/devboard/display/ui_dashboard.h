...
void drawSharedDashboard(T* display, bool is_lcd) {
  ...
  if (soh <= 0)
    soh = 100;  // Default to 100% if no data is provided
  ...
  if (min_cv_info <= 0.0f)
    min_cv_info = 2.80f;  // Default Undervoltage threshold
  if (max_cv_info <= 0.0f)
  ...
  if (cap_kwh <= 0)
    cap_kwh = 30.0;  // Default 30kWh for Fake Battery or missing data
  ...
  if (raw_inv.length() > 12)
    raw_inv = raw_inv.substring(0, 12) + ".";
  if (raw_bat.length() > 12)
    raw_bat = raw_bat.substring(0, 12) + ".";
  ...
  switch (datalayer.battery.status.bms_status) {
    case ACTIVE:
      ...
      break;
    case UPDATING:
      content_sys_stts = "UPDATING";
      break;
    case FAULT:
      content_sys_stts = "FAULT";
      break;
    case INACTIVE:
      content_sys_stts = "INACTIVE";
      break;
    case STANDBY:
      content_sys_stts = "STANDBY";
      break;
    default:
      content_sys_stts = "UNKNOWN";
      break;
  }
  ...
  if (total_cells <= 0)
    total_cells = 16;  // Default to 16 cells
  ...
  if (ov_count > 0) {
    if (ov_cells.endsWith(","))
      ov_cells = ov_cells.substring(0, ov_cells.length() - 1);
    if (ov_count > 3)
      ov_cells += "...";
  } else {
    ov_cells = "None";
  ...
  if (uv_count > 0) {
    if (uv_cells.endsWith(","))
      uv_cells = uv_cells.substring(0, uv_cells.length() - 1);
    if (uv_count > 3)
      uv_cells += "...";
  } else {
    uv_cells = "None";
  ...
  if (delta_mv > 150) {
    has_alarm = true;
    alarm_msg = "WARN: HIGH CELL DELTA (" + String(delta_mv) + "mV)";
  } else if (max_cv > max_cv_info) {
    has_alarm = true;
    alarm_msg = "ALARM: CELL OVERVOLTAGE";
  ...
  } else if (content_sys_stts == "FAULT" || bat_status == "FAULT") {
    has_alarm = true;
    alarm_msg = "FAULT: BMS HAS ERROR!";
  }
  ...
  display->printf("%.1f kW", power_kw);
  display->setFont(f_small);
  display->setCursor(20, 55);
  display->printf(" >> %.1f V", voltage);
  display->setCursor(20, 75);
  display->printf(" >> %.1f A", current);

  if (content_sys_stts == "FAULT")
    display->setTextColor(theme.alert);
  display->setCursor(10, 95);
  display->print("System: " + content_sys_stts);
  display->setTextColor(theme.text);

  if (bat_status == "FAULT")
    display->setTextColor(theme.alert);
  display->setCursor(10, 115);
  display->print("Battery: " + bat_status);
  display->setTextColor(theme.text);
  ...
  int txt_x = (display_soc < 10)    ? bat_x + (bat_w * 0.35)
              : (display_soc < 100) ? bat_x + (bat_w * 0.2)
                                    : bat_x + (bat_w * 0.05);
  ...
  display->setCursor(right_x + 18, 20);
  display->print(": " + ipStr);
  display->setCursor(right_x, 40);
  display->print("INV: " + raw_inv);
  ...
  if (getLocalTime(&timeinfo)) {
    ...
    display->printf("Date: %02d-%02d-%04d", timeinfo.tm_mday, timeinfo.tm_mon + 1, timeinfo.tm_year + 1900);
    display->setCursor(right_x + 10, 120);
  ...
  String max_str =
      (active_cells > 0) ? String(max_cv, 3) + "V [#" + String(max_cell_idx) + "]" : String(max_cv, 3) + "V";
  String min_str =
      (active_cells > 0) ? String(min_cv, 3) + "V [#" + String(min_cell_idx) + "]" : String(min_cv, 3) + "V";
  ...
  if (active_cells > 0) {
    ...
    display->setTextSize(1);
    display->setTextColor(theme.text);
    display->setCursor(gx - 28, gy);
    display->printf("%.1f", graph_max_v);
    display->setCursor(gx - 28, gy + gh - 7);
    display->printf("%.1f", graph_min_v);
