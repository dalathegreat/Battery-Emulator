...
// state. Volatile unsigned ints back a live variable directly wherever possible
// (no callbacks); apply/read hooks are only used when the web value needs
// transformation.
...
// take effect immediately without a reboot. Prefer VolatileStored (a direct
// pointer to the live variable, no callbacks) when the web value is exactly
// what the runtime uses; fall back to VolatileHooked when a transformation or
// validation against live state is required.
