...
String debug_logger_processor(void) {
  ...
  if (!content.reserve(1000 + sizeof(datalayer.system.info.logged_can_messages))) {
    ...
    content += R"rawliteral(
      <div style="padding: 20px;">
        <div style="background: #fff; border-radius: 8px; border-top: 4px solid #e74c3c; padding: 20px; box-shadow: 0 4px 10px rgba(0,0,0,0.05); text-align: center;">
          <h3 style="color: #e74c3c; font-size: 1.5rem; margin-top: 0;">⚠️ Board is Busy! (Out of Memory)</h3>
          <p style="color: #555;">You refreshed too quickly! The board didn't have time to clear the RAM. Please wait 2-3 seconds and try again.</p>
          <button style="background: #3498db; color: #fff; border: none; padding: 10px 20px; border-radius: 6px; font-weight: bold; cursor: pointer; margin-top: 10px;" onclick="location.reload(true)">🔄 Try Again</button>
        </div>
      </div>
    )rawliteral";
  ...
  <script>
    // Automatically push the Terminal screen to the bottom.
