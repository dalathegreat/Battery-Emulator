...
#if !defined(SMALL_FLASH_DEVICE) && __has_include("help_json_autogen.h")
#include "help_json_autogen.h"  // generated by tools/embed_help_json.py
#endif
...
#ifdef HELP_JSON_EMBEDDED
  // The help texts as they were at build time, for when GitHub cannot be reached. Devices with
  // small flash leave it out; there help.js gets a 404 and relies on GitHub alone.
  def_route_with_auth("/help.json", server, HTTP_GET, [](AsyncWebServerRequest* request) {
    AsyncWebServerResponse* response =
        request->beginResponse(200, "application/json", HELP_JSON_GZ, sizeof(HELP_JSON_GZ));
    response->addHeader("Content-Encoding", "gzip");
    request->send(response);
  });
#endif  // HELP_JSON_EMBEDDED
