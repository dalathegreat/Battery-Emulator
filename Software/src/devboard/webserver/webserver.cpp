...
void init_webserver() {
  ...
  // Route for resetting contactors on Stellantis eCMP
  server.on("/ContactorResetStellantis", HTTP_GET, [](AsyncWebServerRequest* request) {
    if (WEBSERVER_AUTH_REQUIRED && !request->authenticate(http_username, http_password)) {
      return request->requestAuthentication();
    }
    datalayer_extended.stellantisECMP.UserRequestContactorReset = true;
    request->send(200, "text/plain", "Updated successfully");
  });
  ...
  server.on("/ContactorResetStellantis", HTTP_GET, [](AsyncWebServerRequest* request) {
    ...
    datalayer_extended.stellantisECMP.UserRequestCollisionReset = true;
    request->send(200, "text/plain", "Updated successfully");
  });
