...
constexpr VolatileUint VolatileStored(const char* name, uint32_t min, uint32_t max, auto* storage) {
  VolatileUint s{};
  s.name = name;
  s.min = min;
  s.max = max;
  s.width = static_cast<uint8_t>(sizeof(*storage));
  s.storage = storage;
  return s;
}
constexpr VolatileUint VolatileHooked(const char* name, uint32_t min, uint32_t max, void (*apply)(uint32_t),
                                      uint32_t (*read)()) {
  VolatileUint s{};
  s.name = name;
  s.min = min;
