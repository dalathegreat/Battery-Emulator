...
void BydModbusInverter::handle_update_data_modbusp201_byd() {
  if (battery2) {
    mbPV[202] = std::min(datalayer.battery.info.total_capacity_Wh + datalayer.battery2.info.total_capacity_Wh,
                         static_cast<uint32_t>(11040u));  //Cap to 11kWh
  } else {
    mbPV[202] = std::min(datalayer.battery.info.total_capacity_Wh, static_cast<uint32_t>(11040u));  //Cap to 11kWh
  }
...
void BydModbusInverter::handle_update_data_modbusp301_byd() {
  ...
  if (battery2) {
    ...
    mbPV[304] = std::min(datalayer.battery.info.total_capacity_Wh, static_cast<uint32_t>(11040u));  //Cap to 11kWh
  }
  if (battery2) {
