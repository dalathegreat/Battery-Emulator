...
void update_values_can_sma_tripower() {  //This function maps all the values fetched from battery CAN to the inverter CAN
  ...
  ampere_hours_remaining = ((datalayer.battery.status.remaining_capacity_W / system_battery_voltage_dV) *
                            100);  //(WH[10000] * V+1[3600])*100 = 270 (27.0Ah)
  ampere_hours_max = ((datalayer.battery.info.total_capacity_Wh / system_battery_voltage_dV) *
                      100);  //(WH[10000] * V+1[3600])*100 = 270 (27.0Ah)
