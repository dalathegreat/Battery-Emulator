...
void FoxessEpCanInverter::transmit_cell_voltage_frame(
    uint32_t frame_id,
    uint16_t first_cell_index) {
  FOXESS_CELLVOLTAGES.ID = frame_id;

  const uint16_t foxess_ep_reported_cell_count =
    datalayer.battery.info.number_of_cells;

const uint16_t foxess_ep_cell_count =
    foxess_ep_reported_cell_count > MAX_AMOUNT_CELLS
        ? MAX_AMOUNT_CELLS
        : foxess_ep_reported_cell_count;
  ...
  if (foxess_ep_cell_count > 0U) {
    const uint16_t final_cell_voltage_mV =
        datalayer.battery.status
            .cell_voltages_mV[foxess_ep_cell_count - 1U];

    if (final_cell_voltage_mV > 0U) {
      foxess_ep_fallback_cell_voltage_mV =
          final_cell_voltage_mV;
    }
  }

  for (uint8_t cell_slot = 0U;
       cell_slot < 4U;
       ++cell_slot) {
    const uint16_t virtual_cell_index =
    first_cell_index + cell_slot;
...
if (foxess_ep_cell_count > 0U) {
  source_cell_index =
      static_cast<uint16_t>(
          (static_cast<uint32_t>(virtual_cell_index) *
           foxess_ep_cell_count) /
          144U);
  ...
  if (source_cell_index >= foxess_ep_cell_count) {
    ...
    if (source_cell_index < foxess_ep_cell_count) {
      const uint16_t live_cell_voltage_mV =
          datalayer.battery.status
              .cell_voltages_mV[source_cell_index];
    ...
    const uint8_t payload_index =
        static_cast<uint8_t>(cell_slot * 2U);

    FOXESS_CELLVOLTAGES.data.u8[payload_index] =
        static_cast<uint8_t>(cell_voltage_mV);

    FOXESS_CELLVOLTAGES.data.u8[payload_index + 1U] =
        static_cast<uint8_t>(cell_voltage_mV >> 8);
  }

  transmit_can_frame(&FOXESS_CELLVOLTAGES);
}

void FoxessEpCanInverter::transmit_temperature_frame(
    uint32_t frame_id,
  ...
  if (minimum_temperature_dC > maximum_temperature_dC) {
    const int16_t temporary_temperature_dC =
  ...
  for (uint8_t byte_index = 0U;
       byte_index < 8U;
       ++byte_index) {
    const uint8_t virtual_sensor_index =
        first_virtual_sensor_index + byte_index;

    const int16_t temperature_dC =
        generic_temperature_dC[
            virtual_sensor_index % 2U];

    int16_t encoded_temperature =
        (temperature_dC / 10) + 50;
    ...
    FOXESS_CELLTEMPERATURES.data.u8[byte_index] =
        static_cast<uint8_t>(encoded_temperature);
  }
...
const bool foxess_ep_battery_comm_live =
    datalayer.battery.status.CAN_battery_still_alive > 0U;

const bool foxess_ep_battery_ready =
    foxess_ep_battery_comm_live &&
    datalayer.system.status.system_status == ACTIVE &&
    datalayer.system.status.battery_allows_contactor_closing &&
    !foxess_ep_system_fault;

const bool foxess_ep_power_path_active =
    foxess_ep_battery_ready &&
    datalayer.system.status.contactors_engaged == 1U;

    constexpr int32_t FOXESS_EP_ACTIVITY_DEADBAND_dA = 10;

const int32_t foxess_ep_activity_current_dA =
    static_cast<int32_t>(
        datalayer.battery.status.reported_current_dA);

const bool foxess_ep_charging_active =
    foxess_ep_power_path_active &&
    foxess_ep_activity_current_dA >=
        FOXESS_EP_ACTIVITY_DEADBAND_dA;
...
const bool foxess_ep_discharging_active =
  ...
  if (!foxess_energy_counter_initialised ||
    !foxess_ep_power_path_active) {
    foxess_previous_energy_millis = foxess_now_millis;
    foxess_energy_counter_initialised = true;
  } else {
    unsigned long elapsed_ms =
        foxess_now_millis - foxess_previous_energy_millis;
    foxess_previous_energy_millis = foxess_now_millis;

    int32_t signed_current_dA =
        (int32_t)datalayer.battery.status.reported_current_dA;
    ...
    uint64_t energy_increment =
        (uint64_t)datalayer.battery.status.voltage_dV *
        (uint64_t)absolute_current_dA *
        (uint64_t)elapsed_ms;
  ...
  foxess_charged_capacity_remainder_dAms +=
      static_cast<uint64_t>(signed_current_dA) *
      static_cast<uint64_t>(elapsed_ms);

  const uint64_t completed_charged_dAh =
      foxess_charged_capacity_remainder_dAms /
      FOXESS_DAH_DIVISOR_dAms;

  if (completed_charged_dAh > 0ULL) {
    foxess_charged_capacity_dAh +=
        completed_charged_dAh;
  ...
  foxess_discharged_capacity_remainder_dAms +=
      static_cast<uint64_t>(absolute_current_dA) *
      static_cast<uint64_t>(elapsed_ms);

  const uint64_t completed_discharged_dAh =
      foxess_discharged_capacity_remainder_dAms /
      FOXESS_DAH_DIVISOR_dAms;

  if (completed_discharged_dAh > 0ULL) {
    foxess_discharged_capacity_dAh +=
        completed_discharged_dAh;
...
if (foxess_ep_battery_ready) {
  foxess_ep_max_charge_current_dA =
      datalayer.battery.status.max_charge_current_dA;

  foxess_ep_max_discharge_current_dA =
...
FOXESS_1873.data.u8[0] =
    (uint8_t)datalayer.battery.status.voltage_dV;
FOXESS_1873.data.u8[1] =
    (uint8_t)(datalayer.battery.status.voltage_dV >> 8);
...
int16_t foxess_ep_current_dA_signed =
    (int16_t)foxess_ep_current_dA;

FOXESS_1873.data.u8[2] =
    (uint8_t)foxess_ep_current_dA_signed;
FOXESS_1873.data.u8[3] =
    (uint8_t)(foxess_ep_current_dA_signed >> 8);

// Bytes 4-5: SOC in whole percent.
uint16_t foxess_ep_pack_soc_percent =
    datalayer.battery.status.reported_soc / 100U;

if (foxess_ep_pack_soc_percent > 100U) {
  foxess_ep_pack_soc_percent = 100U;
}

FOXESS_1873.data.u8[4] =
    static_cast<uint8_t>(foxess_ep_pack_soc_percent);
...
const uint32_t foxess_ep_effective_energy_Wh =
    datalayer.battery.info.reported_total_capacity_Wh;

uint32_t foxess_ep_nominal_energy_Wh =
    datalayer.battery.info.total_capacity_Wh;

uint32_t foxess_ep_soh_pptt_for_energy =
...
if (foxess_ep_effective_energy_Wh > 0U) {
  if (foxess_ep_soh_pptt_for_energy > 0U) {
    foxess_ep_nominal_energy_Wh =
        static_cast<uint32_t>(
            (static_cast<uint64_t>(
                 foxess_ep_effective_energy_Wh) *
                 10000ULL +
             (foxess_ep_soh_pptt_for_energy / 2U)) /
...
uint32_t foxess_ep_nominal_energy_10Wh =
    foxess_ep_nominal_energy_Wh / 10U;

if (foxess_ep_nominal_energy_10Wh > 65535U) {
  foxess_ep_nominal_energy_10Wh = 65535U;
}
FOXESS_1873.data.u8[6] =
    static_cast<uint8_t>(foxess_ep_nominal_energy_10Wh);

FOXESS_1873.data.u8[7] =
    static_cast<uint8_t>(
        foxess_ep_nominal_energy_10Wh >> 8);

  // BMS_CellData - 0x1874

// Bytes 0-1: maximum battery temperature in 0.1 C.
FOXESS_1874.data.u8[0] =
    (uint8_t)datalayer.battery.status.temperature_max_dC;
FOXESS_1874.data.u8[1] =
    (uint8_t)(datalayer.battery.status.temperature_max_dC >> 8);

// Bytes 2-3: minimum battery temperature in 0.1 C.
FOXESS_1874.data.u8[2] =
    (uint8_t)datalayer.battery.status.temperature_min_dC;
FOXESS_1874.data.u8[3] =
    (uint8_t)(datalayer.battery.status.temperature_min_dC >> 8);
...
const uint16_t foxess_ep_reported_cell_count_for_extrema =
    datalayer.battery.info.number_of_cells;

const uint16_t foxess_ep_cell_count_for_extrema =
    foxess_ep_reported_cell_count_for_extrema > MAX_AMOUNT_CELLS
        ? MAX_AMOUNT_CELLS
        : foxess_ep_reported_cell_count_for_extrema;

for (uint16_t cell_index = 0U;
     cell_index < foxess_ep_cell_count_for_extrema;
     ++cell_index) {

      const uint16_t cell_voltage_mV =
    datalayer.battery.status.cell_voltages_mV[cell_index];

  if (cell_voltage_mV == 0) {
    continue;
  }

  if (foxess_ep_max_cell_position == 0 ||
      cell_voltage_mV > foxess_ep_max_cell_voltage_mV) {
    foxess_ep_max_cell_voltage_mV = cell_voltage_mV;
    foxess_ep_max_cell_position =
        static_cast<uint8_t>(cell_index + 1);
  }

  if (foxess_ep_min_cell_position == 0 ||
      cell_voltage_mV < foxess_ep_min_cell_voltage_mV) {
    foxess_ep_min_cell_voltage_mV = cell_voltage_mV;
    foxess_ep_min_cell_position =
        static_cast<uint8_t>(cell_index + 1);
  }
}
...
if (foxess_ep_cell_count_for_extrema > 0U) {
  if (foxess_ep_max_cell_position > 0U) {
    foxess_ep_virtual_max_cell_position =
        static_cast<uint16_t>(
            ((static_cast<uint32_t>(
                  foxess_ep_max_cell_position - 1U) *
              144U +
              foxess_ep_cell_count_for_extrema - 1U) /
             foxess_ep_cell_count_for_extrema) +
            1U);
  }

  if (foxess_ep_min_cell_position > 0U) {
    foxess_ep_virtual_min_cell_position =
        static_cast<uint16_t>(
            ((static_cast<uint32_t>(
                  foxess_ep_min_cell_position - 1U) *
              144U +
              foxess_ep_cell_count_for_extrema - 1U) /
             foxess_ep_cell_count_for_extrema) +
            1U);
...
FOXESS_1874.data.u8[4] =
    static_cast<uint8_t>(foxess_ep_virtual_max_cell_position);
FOXESS_1874.data.u8[5] = 0x00;

FOXESS_1874.data.u8[6] =
    static_cast<uint8_t>(foxess_ep_virtual_min_cell_position);
...
FOXESS_1875.data.u8[4] =
    datalayer.system.status.contactors_engaged
        ? 0x01
        : 0x00;
  FOXESS_1875.data.u8[5] = (uint8_t)0;     //0 Confirmed Unused in Battery Details page

const uint64_t foxess_cycle_charged_energy_Wh =
    foxess_installation_charged_energy_Wh;

const uint64_t foxess_cycle_discharged_energy_Wh =
    foxess_installation_discharged_energy_Wh;


const uint64_t foxess_cycle_throughput_Wh =
    foxess_cycle_charged_energy_Wh +
    foxess_cycle_discharged_energy_Wh;

const uint64_t foxess_cycle_denominator_Wh =
    static_cast<uint64_t>(
        datalayer.battery.info.total_capacity_Wh) *
    2ULL;
...
if (foxess_ep_battery_ready &&
    foxess_cycle_denominator_Wh > 0ULL) {
  foxess_equivalent_cycles =
      foxess_cycle_throughput_Wh /
      foxess_cycle_denominator_Wh;
}
...
FOXESS_1875.data.u8[6] =
    static_cast<uint8_t>(foxess_equivalent_cycles);

FOXESS_1875.data.u8[7] =
    static_cast<uint8_t>(
        foxess_equivalent_cycles >> 8);
  ...
  if (!foxess_ep_battery_ready ||
    datalayer.battery.status.max_charge_current_dA == 0U ||
...
if (foxess_ep_system_fault) {    FOXESS_1877.data.u8[0] = (uint8_t)0x02;
  } else {
...
uint16_t foxess_soh_percent =
    datalayer.battery.status.soh_pptt / 100U;

if (foxess_soh_percent > 100U) {
  foxess_soh_percent = 100U;
}

FOXESS_187B.data.u8[0] =
    static_cast<uint8_t>(foxess_soh_percent);
...
const uint32_t foxess_min_design_voltage_dV =
    datalayer.battery.info.min_design_voltage_dV;

const uint32_t foxess_max_design_voltage_dV =
    datalayer.battery.info.max_design_voltage_dV;

uint32_t foxess_nominal_voltage_dV = 0U;

if (foxess_min_design_voltage_dV > 0U &&
    foxess_max_design_voltage_dV >=
        foxess_min_design_voltage_dV) {
...
const uint32_t foxess_rated_energy_Wh =
    datalayer.battery.info.total_capacity_Wh > 0U
        ? datalayer.battery.info.total_capacity_Wh
        : datalayer.battery.info.reported_total_capacity_Wh;

// Full/inverter-visible energy prefers the reported capacity.
const uint32_t foxess_full_energy_Wh =
    datalayer.battery.info.reported_total_capacity_Wh > 0U
        ? datalayer.battery.info.reported_total_capacity_Wh
        : foxess_rated_energy_Wh;

const bool foxess_full_capacity_valid =
    foxess_nominal_voltage_dV > 0U &&
    foxess_rated_energy_Wh > 0U &&
    foxess_full_energy_Wh > 0U;
  ...
  foxess_rated_capacity_dAh =
      static_cast<uint32_t>(
          (static_cast<uint64_t>(
               foxess_rated_energy_Wh) *
               100ULL +
           (foxess_nominal_voltage_dV / 2U)) /
          foxess_nominal_voltage_dV);

  foxess_full_capacity_dAh =
      static_cast<uint32_t>(
          (static_cast<uint64_t>(
               foxess_full_energy_Wh) *
               100ULL +
           (foxess_nominal_voltage_dV / 2U)) /
          foxess_nominal_voltage_dV);
}
...
FOXESS_187B.data.u8[4] =
    static_cast<uint8_t>(foxess_rated_capacity_dAh);
FOXESS_187B.data.u8[5] =
    static_cast<uint8_t>(foxess_rated_capacity_dAh >> 8);

// Reported full/effective capacity, little-endian.
FOXESS_187B.data.u8[6] =
    static_cast<uint8_t>(foxess_full_capacity_dAh);
FOXESS_187B.data.u8[7] =
    static_cast<uint8_t>(foxess_full_capacity_dAh >> 8);

    // EP capacity summary - 0x1900
// Bytes 0-1: rated battery capacity in 0.1 Ah.
FOXESS_1900.data.u8[0] =
    (uint8_t)foxess_rated_capacity_dAh;
FOXESS_1900.data.u8[1] =
    (uint8_t)(foxess_rated_capacity_dAh >> 8);

// Bytes 2-3: nominal battery energy in 10 Wh units.
FOXESS_1900.data.u8[2] =
    (uint8_t)foxess_ep_nominal_energy_10Wh;
FOXESS_1900.data.u8[3] =
    (uint8_t)(foxess_ep_nominal_energy_10Wh >> 8);
...
FOXESS_1902.data.u8[0] =
    static_cast<uint8_t>(foxess_ep_model_coefficient);

FOXESS_1902.data.u8[1] =
    static_cast<uint8_t>(foxess_ep_model_coefficient >> 8);
...
if (foxess_ep_battery_ready &&
    datalayer.battery.status.max_discharge_current_dA > 0U) {
  foxess_ep_max_discharge_power_1902_W =
...
FOXESS_1902.data.u8[2] =
    static_cast<uint8_t>(
        foxess_ep_max_discharge_power_1902_W);

FOXESS_1902.data.u8[3] =
    static_cast<uint8_t>(
        foxess_ep_max_discharge_power_1902_W >> 8);
...
FOXESS_1902.data.u8[4] =
    static_cast<uint8_t>(foxess_ep_equivalent_resistance_0p1mOhm);

FOXESS_1902.data.u8[5] =
    static_cast<uint8_t>(foxess_ep_equivalent_resistance_0p1mOhm >> 8);

    // Bytes 6-7 : filtered battery/model temperature (whole Â°C)

int32_t foxess_ep_filtered_temperature_1902_C =
    (static_cast<int32_t>(
         datalayer.battery.status.temperature_max_dC) +
     static_cast<int32_t>(
         datalayer.battery.status.temperature_min_dC)) /
...
} else if (foxess_ep_filtered_temperature_1902_C >
           65535) {
  foxess_ep_filtered_temperature_1902_C = 65535;
}

const uint16_t foxess_ep_filtered_temperature_1902_raw =
    static_cast<uint16_t>(
        foxess_ep_filtered_temperature_1902_C);

FOXESS_1902.data.u8[6] =
    static_cast<uint8_t>(
        foxess_ep_filtered_temperature_1902_raw);

FOXESS_1902.data.u8[7] =
    static_cast<uint8_t>(
        foxess_ep_filtered_temperature_1902_raw >> 8);
...
uint32_t foxess_ep_nominal_energy_200Wh =
    foxess_ep_nominal_energy_Wh / 200U;

if (foxess_ep_nominal_energy_200Wh > 65535U) {
  foxess_ep_nominal_energy_200Wh = 65535U;
}

FOXESS_1903.data.u8[6] =
    static_cast<uint8_t>(foxess_ep_nominal_energy_200Wh);

FOXESS_1903.data.u8[7] =
    static_cast<uint8_t>(foxess_ep_nominal_energy_200Wh >> 8);
...
FOXESS_1904.data.u8[0] =
    static_cast<uint8_t>(
        foxess_ep_virtual_max_cell_position);

FOXESS_1904.data.u8[1] = 0x00;

FOXESS_1904.data.u8[2] =
    static_cast<uint8_t>(
        foxess_ep_virtual_min_cell_position);
...
FOXESS_1904.data.u8[5] =
    (configured_number_of_modules == 2) ? 0x12 : 0x0A;
...
FOXESS_1905.data.u8[0] =
    static_cast<uint8_t>(
        foxess_ep_nominal_cell_voltage_1905_10mV);

FOXESS_1905.data.u8[1] =
    static_cast<uint8_t>(
        foxess_ep_nominal_cell_voltage_1905_10mV >> 8);

// Byte 2: primary whole-percent SOC.
// Genuine EP12 sends zero until capacity data is valid.
uint16_t foxess_ep_primary_soc_1905 =
    datalayer.battery.status.reported_soc / 100U;
...
const bool foxess_ep_capacity_model_ready =
    foxess_ep_battery_ready &&
    foxess_full_capacity_valid;

if (!foxess_ep_capacity_model_ready) {
  foxess_ep_primary_soc_1905 = 0U;
}

FOXESS_1905.data.u8[2] =
    static_cast<uint8_t>(foxess_ep_primary_soc_1905);

// Byte 3: SOC/capacity-model state.
FOXESS_1905.data.u8[3] =
    foxess_ep_capacity_model_ready ? 0x08 : 0x04;
...
FOXESS_1906.data.u8[0] =
    static_cast<uint8_t>(foxess_ep_model_parameter_a_1906);

FOXESS_1906.data.u8[1] =
    static_cast<uint8_t>(foxess_ep_model_parameter_a_1906 >> 8);
...
FOXESS_1906.data.u8[4] =
    static_cast<uint8_t>(foxess_ep_model_parameter_b_1906);

FOXESS_1906.data.u8[5] =
    static_cast<uint8_t>(foxess_ep_model_parameter_b_1906 >> 8);
...
FOXESS_1905.data.u8[4] =
    static_cast<uint8_t>(foxess_ep_model_parameter_b_1906 / 10U);
...
if (foxess_ep_capacity_model_ready) {
  foxess_ep_fine_soc_permille_1907 =
      datalayer.battery.status.reported_soc / 10U;
...
const uint32_t foxess_ep_voltage_model_state_permille_1907 =
    foxess_ep_fine_soc_permille_1907;

// Bytes 0-3: voltage-sensitive/model battery state.
FOXESS_1907.data.u8[0] =
    static_cast<uint8_t>(foxess_ep_voltage_model_state_permille_1907);

FOXESS_1907.data.u8[1] =
    static_cast<uint8_t>(foxess_ep_voltage_model_state_permille_1907 >> 8);

FOXESS_1907.data.u8[2] =
    static_cast<uint8_t>(foxess_ep_voltage_model_state_permille_1907 >> 16);

FOXESS_1907.data.u8[3] =
    static_cast<uint8_t>(foxess_ep_voltage_model_state_permille_1907 >> 24);

// Bytes 4-7: fine internal SOC/SOE in 0.1%.
FOXESS_1907.data.u8[4] =
    static_cast<uint8_t>(foxess_ep_fine_soc_permille_1907);

FOXESS_1907.data.u8[5] =
    static_cast<uint8_t>(foxess_ep_fine_soc_permille_1907 >> 8);

FOXESS_1907.data.u8[6] =
    static_cast<uint8_t>(foxess_ep_fine_soc_permille_1907 >> 16);

FOXESS_1907.data.u8[7] =
    static_cast<uint8_t>(foxess_ep_fine_soc_permille_1907 >> 24);

// Keep 0x1905 byte 5 consistent with the fine 0x1907 value.
FOXESS_1905.data.u8[5] =
    static_cast<uint8_t>(foxess_ep_fine_soc_permille_1907 / 10U);

    // 0x1908 - capacity-adjusted battery-state model

uint32_t foxess_ep_adjusted_soc_permille_1908 =
    foxess_ep_fine_soc_permille_1907;

if (foxess_ep_capacity_model_ready &&
    foxess_rated_capacity_dAh > 0U) {
  foxess_ep_adjusted_soc_permille_1908 =
      static_cast<uint32_t>(
          (static_cast<uint64_t>(foxess_ep_fine_soc_permille_1907) *
           foxess_full_capacity_dAh) /
          foxess_rated_capacity_dAh);
}
...
const uint32_t foxess_ep_energy_model_state_1908 =
    foxess_ep_capacity_model_ready ? 16U : 14U;

FOXESS_1908.data.u8[0] =
    static_cast<uint8_t>(foxess_ep_energy_model_state_1908);

FOXESS_1908.data.u8[1] =
    static_cast<uint8_t>(foxess_ep_energy_model_state_1908 >> 8);

FOXESS_1908.data.u8[2] =
    static_cast<uint8_t>(foxess_ep_energy_model_state_1908 >> 16);

FOXESS_1908.data.u8[3] =
    static_cast<uint8_t>(foxess_ep_energy_model_state_1908 >> 24);

// Bytes 4-7: fine capacity-adjusted SOC/SOE in 0.1%.
FOXESS_1908.data.u8[4] =
    static_cast<uint8_t>(foxess_ep_adjusted_soc_permille_1908);

FOXESS_1908.data.u8[5] =
    static_cast<uint8_t>(foxess_ep_adjusted_soc_permille_1908 >> 8);

FOXESS_1908.data.u8[6] =
    static_cast<uint8_t>(foxess_ep_adjusted_soc_permille_1908 >> 16);

FOXESS_1908.data.u8[7] =
    static_cast<uint8_t>(foxess_ep_adjusted_soc_permille_1908 >> 24);

// Keep 0x1905 byte 6 consistent with the fine 0x1908 value.
FOXESS_1905.data.u8[6] =
    static_cast<uint8_t>(foxess_ep_adjusted_soc_permille_1908 / 10U);
...
FOXESS_1878.data.u8[1] =
    static_cast<uint8_t>(foxess_ep_unit_soc);
...
const uint32_t foxess_ep_absolute_throughput_Wh_wire =
    foxess_ep_absolute_throughput_Wh > UINT32_MAX
        ? UINT32_MAX
        : static_cast<uint32_t>(
              foxess_ep_absolute_throughput_Wh);

FOXESS_1878.data.u8[4] =
    static_cast<uint8_t>(foxess_ep_absolute_throughput_Wh_wire);

FOXESS_1878.data.u8[5] =
    static_cast<uint8_t>(
        foxess_ep_absolute_throughput_Wh_wire >> 8);

FOXESS_1878.data.u8[6] =
    static_cast<uint8_t>(
        foxess_ep_absolute_throughput_Wh_wire >> 16);

FOXESS_1878.data.u8[7] =
    static_cast<uint8_t>(
        foxess_ep_absolute_throughput_Wh_wire >> 24);
...
const uint32_t foxess_ep_charged_capacity_dAh_wire =
    foxess_charged_capacity_dAh > UINT32_MAX
        ? UINT32_MAX
        : static_cast<uint32_t>(
              foxess_charged_capacity_dAh);

const uint32_t foxess_ep_discharged_capacity_dAh_wire =
    foxess_discharged_capacity_dAh > UINT32_MAX
        ? UINT32_MAX
        : static_cast<uint32_t>(
              foxess_discharged_capacity_dAh);

FOXESS_1879.data.u8[0] =
    static_cast<uint8_t>(
        foxess_ep_charged_capacity_dAh_wire);

FOXESS_1879.data.u8[1] =
    static_cast<uint8_t>(
        foxess_ep_charged_capacity_dAh_wire >> 8);

FOXESS_1879.data.u8[2] =
    static_cast<uint8_t>(
        foxess_ep_charged_capacity_dAh_wire >> 16);

FOXESS_1879.data.u8[3] =
    static_cast<uint8_t>(
        foxess_ep_charged_capacity_dAh_wire >> 24);

FOXESS_1879.data.u8[4] =
    static_cast<uint8_t>(
        foxess_ep_discharged_capacity_dAh_wire);

FOXESS_1879.data.u8[5] =
    static_cast<uint8_t>(
        foxess_ep_discharged_capacity_dAh_wire >> 8);

FOXESS_1879.data.u8[6] =
    static_cast<uint8_t>(
        foxess_ep_discharged_capacity_dAh_wire >> 16);

FOXESS_1879.data.u8[7] =
    static_cast<uint8_t>(
        foxess_ep_discharged_capacity_dAh_wire >> 24);
// Charged energy - 0x187A
// Bytes 0-3: accumulated charged energy in 0.1 kWh units.
const uint64_t foxess_ep_charged_energy_100Wh_raw =
    foxess_installation_charged_energy_Wh / 100ULL;

const uint32_t foxess_ep_charged_energy_100Wh =
    foxess_ep_charged_energy_100Wh_raw > UINT32_MAX
        ? UINT32_MAX
        : static_cast<uint32_t>(
              foxess_ep_charged_energy_100Wh_raw);

// Bytes 4-7: accumulated discharged energy in 0.1 kWh units.
const uint64_t foxess_ep_discharged_energy_100Wh_raw =
    foxess_installation_discharged_energy_Wh / 100ULL;

const uint32_t foxess_ep_discharged_energy_100Wh =
    foxess_ep_discharged_energy_100Wh_raw > UINT32_MAX
        ? UINT32_MAX
        : static_cast<uint32_t>(
              foxess_ep_discharged_energy_100Wh_raw);

FOXESS_187A.data.u8[0] =
    static_cast<uint8_t>(foxess_ep_charged_energy_100Wh);
FOXESS_187A.data.u8[1] =
    static_cast<uint8_t>(foxess_ep_charged_energy_100Wh >> 8);
FOXESS_187A.data.u8[2] =
    static_cast<uint8_t>(foxess_ep_charged_energy_100Wh >> 16);
FOXESS_187A.data.u8[3] =
    static_cast<uint8_t>(foxess_ep_charged_energy_100Wh >> 24);

FOXESS_187A.data.u8[4] =
    static_cast<uint8_t>(foxess_ep_discharged_energy_100Wh);
FOXESS_187A.data.u8[5] =
    static_cast<uint8_t>(foxess_ep_discharged_energy_100Wh >> 8);
FOXESS_187A.data.u8[6] =
    static_cast<uint8_t>(foxess_ep_discharged_energy_100Wh >> 16);
FOXESS_187A.data.u8[7] =
    static_cast<uint8_t>(foxess_ep_discharged_energy_100Wh >> 24);
...
int16_t foxess_ep_max_temperature_encoded =
    (datalayer.battery.status.temperature_max_dC / 10) + 50;

int16_t foxess_ep_min_temperature_encoded =
    (datalayer.battery.status.temperature_min_dC / 10) + 50;
...
temperature_max_per_pack =
    static_cast<uint8_t>(foxess_ep_max_temperature_encoded);

temperature_min_per_pack =
    static_cast<uint8_t>(foxess_ep_min_temperature_encoded);
...
const int16_t foxess_ep_unit_current_signed =
    static_cast<int16_t>(foxess_ep_unit_current_dA);

FOXESS_0C05.data.u8[0] =
    static_cast<uint8_t>(foxess_ep_unit_current_signed);
FOXESS_0C05.data.u8[1] =
    static_cast<uint8_t>(foxess_ep_unit_current_signed >> 8);
...
FOXESS_0C05.data.u8[5] =
    static_cast<uint8_t>(foxess_ep_unit_max_cell_mV);

FOXESS_0C05.data.u8[6] =
    static_cast<uint8_t>(
        ((foxess_ep_unit_max_cell_mV >> 8) & 0x0FU) |
        ((foxess_ep_unit_min_cell_mV & 0x000FU) << 4));

FOXESS_0C05.data.u8[7] =
    static_cast<uint8_t>(foxess_ep_unit_min_cell_mV >> 4);
...
case 4:
  ...
  if (currentMillis - previousMillisIndividualPacks >=
  ...
  if (currentMillis - previousMillisSerialNumber >=
      delay_between_batches_ms) {
...
if (send_celltemperatures) {
  if (currentMillis - previousMillisCelltemperature >=
      delay_between_batches_ms) {
