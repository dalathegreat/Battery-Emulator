...
class CanInverterProtocol : public InverterProtocol, Transmitter, CanReceiver {
 ...
 protected:
  ...
  CAN_Speed requested_speed;

  explicit CanInverterProtocol(CAN_Speed speed = CAN_Speed::CAN_SPEED_500KBPS) : requested_speed(speed) {
    can_interface = can_config.inverter;
    register_transmitter(this);
    register_can_receiver(this, can_interface, requested_speed);
    logging.print("Requesting ");
    logging.print((uint32_t)requested_speed);
