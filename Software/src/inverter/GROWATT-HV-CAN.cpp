...
void GrowattHvInverter::update_values() {
  ...
  // Battery actual capacity (0-100) TODO, what is unit?
  ...
  // and the code for monitoring (communication) is ZEAA
  // TODO, is this needed?
  ...
  GROWATT_3290.data.u8[0] = 0;  // DTC, range [0,65536], Default 12041 (TODO, shall we send that?)
...
void GrowattHvInverter::transmit_can(unsigned long currentMillis) {
  if (!inverter_alive) {
    return;  // Don't send messages towards inverter until it has started
  }
