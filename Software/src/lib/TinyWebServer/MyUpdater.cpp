...
bool MyUpdateClass::_enablePartition(const esp_partition_t *partition) {
  if (!partition) {
    return false;
  }
...
MyUpdateClass &MyUpdateClass::onProgress(THandlerFunction_Progress fn) {
  _progress_callback = fn;
  return *this;
}

void MyUpdateClass::_reset() {
  if (_buffer) {
    delete[] _buffer;
  }
  if (_skipBuffer) {
...
bool MyUpdateClass::canRollBack() {
...
bool MyUpdateClass::begin(size_t size, int command, int ledPin, uint8_t ledOn, const char *label) {
  if (_size > 0) {
...
#ifndef UPDATE_NOCRYPT
bool MyUpdateClass::setupCrypt(const uint8_t *cryptKey, size_t cryptAddress, uint8_t cryptConfig, int cryptMode) {
...
bool MyUpdateClass::setCryptKey(const uint8_t *cryptKey) {
  if (!cryptKey) {
    if (_cryptKey) {
      delete[] _cryptKey;
      _cryptKey = 0;
...
bool MyUpdateClass::setCryptMode(const int cryptMode) {
...
void MyUpdateClass::_abort(uint8_t err) {
  _reset();
  _error = err;
}

void MyUpdateClass::abort() {
  _abort(UPDATE_ERROR_ABORT);
}

#ifndef UPDATE_NOCRYPT
void MyUpdateClass::_cryptKeyTweak(size_t cryptAddress, uint8_t *tweaked_key) {
...
bool MyUpdateClass::_decryptBuffer() {
  if (!_cryptKey) {
...
bool MyUpdateClass::_writeBuffer() {
...
//     return true;
  ...
  if (block_erase || part_head_sectors || part_tail_sectors) {
    //logging.printf("partitionEraseRange: %s, offset: %u, size: %u\n",
    //              _partition->label, _progress, block_erase ? SPI_FLASH_BLOCK_SIZE : SPI_FLASH_SEC_SIZE);
...
bool MyUpdateClass::_verifyHeader(uint8_t data) {
  if (_command == U_FLASH) {
...
bool MyUpdateClass::end(bool evenIfRemaining) {
  if (hasError() || _size == 0) {
    return false;
  }
...
size_t MyUpdateClass::write(uint8_t *data, size_t len) {
  ...
  //logging.printf("doing memcpy, len: %zu, remaining: %zu, sfss %d\n", len, remaining(), SPI_FLASH_SEC_SIZE);
  //return len;
...
size_t MyUpdateClass::writeStream(Stream &data) {
  size_t written = 0;
  size_t toRead = 0;
...
void MyUpdateClass::printError(Print &out) {
  out.println(_err2str(_error));
}

const char *MyUpdateClass::errorString() {
  return _err2str(_error);
}

bool MyUpdateClass::_chkDataInBlock(const uint8_t *data, size_t len) const {
