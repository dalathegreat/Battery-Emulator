...
// flash window" from "INT masked until the window ended". Read either side
// of a window: a top half that never ran leaves the count unchanged, and
// that is the only observable difference - from the task side both cases
// look identical, a drain that arrives late.
