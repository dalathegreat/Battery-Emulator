...
#define MAX_PRECHARGE_TIME_MS \
  15000  // Maximum time precharge may be enabled
...
void handle_precharge_control() {
  ...
  switch (prechargeStatus) {
    case PRECHARGE_IDLE:
      ...
      if (datalayer.battery.status.bms_status == STANDBY  &&
    ...
    case PRECHARGE:
      ...
      if (prev_external_voltage != external_voltage){
        ...
        logging.printf("Precharge: Target: %d V  Extern: %d V  Frequency: %u\n", target_voltage/10, external_voltage/10, freq);
        ledcWriteTone(PRECHARGE_PIN, freq);
      }

      if ((datalayer.battery.status.bms_status != STANDBY && datalayer.battery.status.bms_status != ACTIVE) || datalayer.system.settings.equipment_stop_active) {
        pinMode(PRECHARGE_PIN, OUTPUT);
        digitalWrite(PRECHARGE_PIN, LOW);
    ...
    case PRECHARGE_OFF:
      if (!datalayer.system.status.battery_allows_contactor_closing ||
          !datalayer.system.status.inverter_allows_contactor_closing || datalayer.system.settings.equipment_stop_active || datalayer.battery.status.bms_status != FAULT){
        prechargeStatus = PRECHARGE_IDLE;
        pinMode(PRECHARGE_PIN, OUTPUT);
        digitalWrite(PRECHARGE_PIN, LOW);
        logging.printf("Precharge: equipment stop activated -> IDLE\n");
