...
const uint16_t max_voltage = ABSOLUTE_MAX_VOLTAGE; 
const uint16_t min_voltage = ABSOLUTE_MIN_VOLTAGE;
//Following variables get updated later on from battery CAN communication
uint16_t battery_voltage = 3700; 
uint16_t SOC = 5000; //SOC 0-100.00% 
uint16_t SOH = 9900; //SOH 0-100.00% 
uint16_t capacity_Wh = BATTERY_WH_MAX; 
uint16_t remaining_capacity_Wh = BATTERY_WH_MAX; 
uint16_t max_target_discharge_power = 0; //0W (0W > restricts to no discharge) 
uint16_t max_target_charge_power = 0; //4312 == 4.3kW (during charge), both 307&308 can be set (>0) at the same time 
uint16_t temperature_max = 60; 
uint16_t temperature_min = 50;
uint16_t bms_char_dis_status = 0; //0 idle, 1 discharging, 2, charging
uint16_t bms_status = ACTIVE; //ACTIVE - [STANDBY,INACTIVE,DARKSTART,ACTIVE,FAULT,UPDATING] 
uint16_t stat_batt_power = 0; 
...
{
  #ifdef BATTERY_TYPE_LEAF
  //Runs as fast as possible to check if we get or need to send messages
    ...
    //Update values heading from CAN to Modbus
    #ifdef BATTERY_TYPE_LEAF
...
void handle_update_data_modbusp301() {
	...
	battery_data[10] = battery_voltage;                    // Id.: p311 Value.: 3161 Scaled value.: 316,1VDC Comment.: Batt Voltage inner: 173.2V
