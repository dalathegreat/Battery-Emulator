...
static CAN_frame make_7bb_ff(uint16_t total_len, const uint8_t* first6) {
  ...
  for (int i = 0; i < 6; i++) f.data.u8[2 + i] = first6[i];
  return f;
}
...
static CAN_frame make_7bb_cf(uint8_t sn, const uint8_t* data7) {
  ...
  for (int i = 0; i < 7; i++) f.data.u8[1 + i] = data7[i];
  return f;
}
...
class Smart453Test : public ::testing::Test {
 protected:
  ...
  void feed_pid07(uint8_t contactor, uint8_t hvil, uint8_t svc_disc,
                  uint16_t fusi, uint8_t relay_perm, uint8_t bms_12v) {
    // Build 26-byte ISO-TP payload for PID 0x07
    uint8_t p[26] = {};
    p[0] = 0x61;          // positive response
    p[1] = 0x07;          // PID echo
    // P[3..4] = cell_min (raw)
    p[3] = 0x0E; p[4] = 0x00;   // ~3.5 V
    // P[5..6] = cell_max (raw)
    p[5] = 0x0E; p[6] = 0x80;   // ~3.53 V
    // P[7..8] = dc_link (raw)
    p[7] = 0x55; p[8] = 0x00;   // ~341 V
    // P[9..10] = pack_sum (raw)
    p[9] = 0x55; p[10] = 0x00;
...
TEST_F(Smart453Test, CellVoltageScaling) {
  ...
  if (remaining > 0) memcpy(last_cf, payload + last_offset, remaining);
...
TEST_F(Smart453Test, CurrentSignedDecoding) {
  ...
  uint8_t p[26] = {};
  p[0] = 0x61; p[1] = 0x07;
  p[13] = 0x02; p[14] = 0x00;  // positive current
  p[9] = 0x55; p[10] = 0x00;   // pack voltage raw
  p[16] = 1; p[17] = 1;        // interlocks valid
  p[23] = (1 << 6) | (1 << 4); // relay_permit=1, relay_status=1

  uint8_t ff6[6]; memcpy(ff6, p, 6);
  bat->handle_incoming_can_frame(make_7bb_ff(26, ff6));
  uint8_t cf1[7]; memcpy(cf1, p + 6, 7);
  bat->handle_incoming_can_frame(make_7bb_cf(1, cf1));
  uint8_t cf2[7]; memcpy(cf2, p + 13, 7);
  bat->handle_incoming_can_frame(make_7bb_cf(2, cf2));
  uint8_t cf3[7] = {}; memcpy(cf3, p + 20, 6);
  bat->handle_incoming_can_frame(make_7bb_cf(3, cf3));
...
TEST_F(Smart453Test, CurrentNegativeDecoding) {
  ...
  uint8_t p[26] = {};
  p[0] = 0x61; p[1] = 0x07;
  p[13] = 0xFF; p[14] = 0x00;  // negative (signed)
  p[9] = 0x55; p[10] = 0x00;
...
TEST_F(Smart453Test, PackVoltage_OutOfRange_NotAccepted) {
  ...
  uint8_t p[26] = {};
  p[0] = 0x61; p[1] = 0x07;
  p[9] = 0x7D; p[10] = 0x00;  // pack sum raw = 32000
  ...
  uint8_t cf2[7] = {}; memcpy(cf2, p + 13, 7);
  bat->handle_incoming_can_frame(make_7bb_cf(2, cf2));
  uint8_t cf3[7] = {}; bat->handle_incoming_can_frame(make_7bb_cf(3, cf3));
...
TEST_F(Smart453Test, BroadcastVoltage_0x0C6_DecodedCorrectly) {
  ...
  f.data.u8[0] = 0x7F; f.data.u8[1] = 0xE1;
  // bytes 2-3: current (not decoded yet)
  f.data.u8[2] = 0x80; f.data.u8[3] = 0x00;
  // bytes 4-5: DC link 0x7FF7 = 32759
  f.data.u8[4] = 0x7F; f.data.u8[5] = 0xF7;
  f.data.u8[6] = 0xB8; f.data.u8[7] = 0xF0;
...
TEST_F(Smart453Test, UDS_Voltage_OverridesBroadcast) {
  ...
  CAN_frame bc = {};
  bc.ID = 0x0C6; bc.DLC = 8;
  bc.data.u8[0] = 0x7F; bc.data.u8[1] = 0xE1;  // 32737
  bc.data.u8[4] = 0x7F; bc.data.u8[5] = 0xF7;
...
TEST_F(Smart453Test, BroadcastVoltage_OutOfRange_Rejected) {
  ...
  CAN_frame f = {};
  f.ID = 0x0C6; f.DLC = 8;
  f.data.u8[0] = 0x0F; f.data.u8[1] = 0xFF;  // 4095 < 20000: rejected
  f.data.u8[4] = 0x0F; f.data.u8[5] = 0xFF;
  bat->handle_incoming_can_frame(f);
  bat->update_values();
...
TEST_F(Smart453Test, BroadcastPrecharge_0x090_Invalid_FF_Cleared) {
  ...
  CAN_frame f = {};
  f.ID = 0x090; f.DLC = 7;
  f.data.u8[0] = 90; f.data.u8[1] = 0xFF;
  bat->handle_incoming_can_frame(f);
...
TEST_F(Smart453Test, Temperature_Bit15_OffsetCorrectionApplied) {
  ...
  uint8_t p[12] = {};
  p[0] = 0x61; p[1] = 0x04;
  p[2] = 0x05; p[3] = 0x00;   // [0] min: 0x0500 = 20°C
  p[4] = 0x8F; p[5] = 0x00;   // [1] max with bit 15: 0x8F00 → cleared to 0x0F00 = 60°C
  p[6] = 0x0A; p[7] = 0x00;   // [2] avg: 0x0A00 = 40°C
  p[8] = 0x0F; p[9] = 0x00;   // [3] individual: 0x0F00 → after -0x0A00 = 0x0500 = 20°C
  p[10] = 0x0F; p[11] = 0x00; // [4] individual: same

  uint8_t ff6[6]; memcpy(ff6, p, 6);
  bat->handle_incoming_can_frame(make_7bb_ff(total, ff6));
  uint8_t cf1[7] = {}; memcpy(cf1, p + 6, 6);
  bat->handle_incoming_can_frame(make_7bb_cf(1, cf1));
  bat->update_values();
