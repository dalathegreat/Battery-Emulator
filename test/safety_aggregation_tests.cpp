...
class SafetyAggregationTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    // #2832 gates the charge/discharge limit checks on this global too, and it
    // lives outside the datalayer, so it carries whatever a previous test left.
...
TEST_F(SafetyAggregationTest, OverheatSetsAboveThresholdAndClearsBelow) {
  ...
  // The event carries the temperature itself. Not cast to uint8_t: the data
  // field is int16_t, and truncating here would have this pass against a
  // wrapped 254 for a 51.0 degree reading.
  EXPECT_EQ(get_event_pointer(EVENT_BATTERY_OVERHEAT)->data, kOverheatDeciC);
...
TEST_F(SafetyAggregationTest, TempDeviationFollowsThePack) {
  ...
  // This warning FOLLOWS the pack rather than latching, and that is deliberate
  // upstream: the else branch below it calls clear_event(), which only acts on
  // an ACTIVE event, so a latched variant could never be released and the
  // warning would stick for the rest of the boot after one transient spread.
  // Pinned in both directions so neither half can be lost - re-latching it
  // would silently restore that trap.
  datalayer.battery.status.temperature_max_dC = 200;
  datalayer.battery.status.temperature_min_dC = 190;
  update_machineryprotection();
  EXPECT_FALSE(event_active(EVENT_BATTERY_TEMP_DEVIATION_HIGH));
}
