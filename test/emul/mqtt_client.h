...
typedef enum esp_mqtt_event_id_t {
  MQTT_EVENT_ANY = -1,
  MQTT_EVENT_ERROR = 0,      /*!< on error event, additional context: connection return code, error
...
typedef struct esp_mqtt_error_codes {
  ...
  esp_mqtt_connect_return_code_t connect_return_code; /*!< connection refused error code reported from
...
typedef struct esp_mqtt_event_t {
  ...
  esp_mqtt_error_codes_t* error_handle; /*!< esp-mqtt error handle including esp-tls errors as well
...
typedef struct esp_mqtt_client_config_t {
  ...
  struct network_t {
    ...
    esp_transport_handle_t
        transport; /*!< Custom transport handle to use. Warning: The transport should be valid during the client lifetime and is destroyed when esp_mqtt_client_destroy is called. */
    struct ifreq*
        if_name; /*!< The name of interface for data to go through. Use the default interface without setting */
...
#define esp_mqtt_client_subscribe(client_handle, topic_type, qos_or_size) \
  _Generic((topic_type),                                                  \
      ...
      esp_mqtt_topic_t*: esp_mqtt_client_subscribe_multiple)(client_handle, topic_type, qos_or_size)
